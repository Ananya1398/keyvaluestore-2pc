...
public class ClientOne {
  ...
  public static void main(String[] args) {
    ...
    String participantName = availableServers.get(port);
    ...
    try {
      ...
      ParticipantInterface stub = (ParticipantInterface) registry.lookup(participantName);
      log("Connected to RMI server: " + participantName + " on port: " + port);
      prepopulateData(stub, participantName);
      executeCommands(stub, participantName);
      ...
      while (true) {
        ...
        executor.submit(() -> {
          String response = processCommand(stub, input, participantName);
   ...
   */
  private static void prepopulateData(ParticipantInterface stub, String participantName) {
    ...
    for (String operation : prepopulatedData) {
      executor.submit(() -> {
        String response = processCommand(stub, operation, participantName);
   ...
   * @param stub The RMI remote object.
   */
  private static void executeCommands(ParticipantInterface stub, String participantName) {
    ...
    for (String operation : executeCommands) {
      executor.submit(() -> {
        String response = processCommand(stub, operation, participantName);
        log("Response from server for executing batch: " + response);
      });
    }
   ...
   */
  private static String processCommand(ParticipantInterface stub, String command, String participantName) {
    ...
    try {
      switch (operation.toLowerCase()) {
        case "put":
          ...
          return stub.put(key, arguments[2], CLIENT_NAME, participantName);
        case "get":
          return stub.get(key, CLIENT_NAME, participantName);
        case "delete":
          return stub.delete(key, CLIENT_NAME, participantName);
        default:
